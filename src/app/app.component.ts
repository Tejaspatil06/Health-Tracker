...
@Component({
    ...
    <div class="container">
      ...
      <form #workoutForm="ngForm" (ngSubmit)="onSubmit(workoutForm)">
        <div class="form-group">
          ...
          <input type="text" id="userName" name="userName" [(ngModel)]="userName" placeholder="Enter User Name" required>
        </div>
        <div class="form-row">
          <div class="form-group">
  ...
  `,
  styles: [`
    .container { max-width: 1000px; margin: 0 auto; padding: 20px; font-family: Arial, sans-serif; background-color: #0B051D; color: white; }
    h1 { background: linear-gradient(90deg, #6a11cb, #2575fc); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    .form-group { margin-bottom: 15px; }
    .form-row { display: flex; gap: 15px; }
    .form-row .form-group { flex: 1; }
    label { display: block; margin-bottom: 5px; }
    input, select { width: 100%; padding: 8px; border: 1px solid #ddd; border-radius: 4px; box-sizing: border-box; color: black; }
    button { padding: 10px 20px; background-color: #007bff; color: white; border: none; border-radius: 4px; cursor: pointer; }
    button:disabled { background-color: #2575fc; }
    .workout-list { margin-top: 30px; }
    .filters { display: flex; gap: 15px; margin-bottom: 15px; }
    table { width: 100%; border-collapse: collapse; margin-bottom: 15px; }
    th, td { border: 1px solid #ddd; padding: 8px; text-align: left; color: white; }
    th { background-color: #2575fc; }
    .pagination { display: flex; justify-content: space-between; align-items: center; margin-top: 15px; color: white; }
    ...
    .user-list li { padding: 10px; cursor: pointer; border-bottom: 1px solid #ddd; color: white; }
    .user-list li.selected { background-color: #2575fc; }
      ...
      this.chart = new Chart(canvas, {
        ...
        data: {
          ...
          datasets: [{
            ...
            data: data,
            backgroundColor: 'rgb(63, 224, 235)',
